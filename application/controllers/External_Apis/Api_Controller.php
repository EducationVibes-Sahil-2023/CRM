...
class Api_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (str_contains($current_url, 'login') || str_contains($current_url, 'process_data') || str_contains($current_url, 'call_activity_cron') ||  str_contains($current_url, "update_all_contacts") ||  str_contains($current_url, "excel_sync") || str_contains($current_url, "applicant_sync_excel") || str_contains($current_url, "quotations") || str_contains($current_url, "payment_dues") || str_contains($current_url, "update_quotations")  || str_contains($current_url, "check_lead_auto_transfer_lead")  || str_contains($current_url, "update_excelData")  || str_contains($current_url, "transfer_whatsapp_notification")  || str_contains($current_url, "google_qualified_leads")  || str_contains($current_url, "check_lead_auto_assignation_lead") || str_contains($current_url, "weekend_lead_assignation") || str_contains($current_url, "cron/") ) {
        } else {
            if (!empty($token)) {
                $token = explode(" ", $token);
                if (!empty($token[1])) {
