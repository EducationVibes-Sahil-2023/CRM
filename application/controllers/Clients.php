...
class Clients extends ClientsController
{
    ...
    public function index()
    {
        ...
        // print_r($basicDetailsStatus);
        $admissionPreferencesStatus = $basicDetails->admission_preferences_status;
        $academicDetailsStatus = $basicDetails->academic_details_status;
        $documentDetailsStatus = $basicDetails->document_details_status;
        $declarationDetailsStatus = $basicDetails->declaration_details_status;
    ...
    {
        if ($this->input->post()) {
            $this->form_validation->set_rules('first_name', _l('client_firstname'), 'required');
            $this->form_validation->set_rules('father_name', _l('client_fathersname'), 'required');
    ...
    {
        if ($this->input->post()) {
            $this->form_validation->set_rules('program', _l('Program'), 'required');
    ...
    {
        if ($this->input->post()) {
            $this->form_validation->set_rules('fname', _l('Father Name'), 'required');
            $this->form_validation->set_rules('mname', _l('Mother Name'), 'required');
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $parentDetailsid = $this->clients_model->addParentDetails([
                    ...
                    // 'fdesignation' => $data['fdesignation'],
                    'fannual_income' => $data['fannual_income'],
                    'mtitle' => $data['mtitle'],
                    'mname' => $data['mname'],
                    'memail' => $data['memail'],
                    'mmobile' => $data['mmobile'],
                    'moccupation' => $data['moccupation'],
                    // 'mdesignation' => $data['mdesignation'],
                    'mannual_income' => $data['mannual_income'],
                    // 'gtitle' => $data['gtitle'],
                    // 'gname' => $data['gname'],
                    // 'gemail' => $data['gemail'],
                    // 'gmobile' => $data['gmobile'],
                    // 'goccupation' => $data['goccupation'],
                    // 'gdesignation' => $data['gdesignation'],
                    // 'gannual_income' => $data['gannual_income'],
    ...
    public function upload_docs_new()
    {
        ...
        if (!empty($_POST)) {
            ...
            {
                // $this->db->update(tblacadmic_documents)
            }
