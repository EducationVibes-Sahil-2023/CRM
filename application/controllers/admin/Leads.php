...
$data['lead_sub_status'] = $this->leads_model->lead_sub_status();
        ...
        // echo "<pre>";print_r($data['team']);die;
...
// }
    ...
    public function mk()
    {
        ...
        $type   = $this->staff_model->get_team();
    ...
    public function add_lead_transfer_request()
    {
        try {
            ...
            $type = $this->input->post('transfer_lead_type');
            $assigned = $this->input->post('transfer_lead_assign');
            ...
            if (!$check_lead_transfer_request) {
                // Insert new lead transfer request
                ...
                if (!empty($transfer_lead_id)) {
                    if (is_admin()) {
                        // Add approval fields if admin
                        if (!empty($type) && !empty($assigned)) {
                            $data = array_merge($data, [
                                "updated_by" => get_staff_user_id(),
                                "updated_at" => date('Y-m-d H:i:s'),
                    ...
                    $update_ = $this->db->update(db_prefix() . 'lead_transfer_request', $data, "id = " . $transfer_lead_id);
                    $this->lead_transfer_notification($lead_id, 2);
                    if ($update_) {
                        $message = "Lead transfer request updated successfully.";
                        $success = true;
                    } else {
                        throw new Exception("Failed to update lead transfer request.");
                    }
                } else {
    ...
    public function lead_transfer_notification($lead_id, $status = '')
    {
        $check_lead_transfer_request = $this->leads_model->get_lead_transfer_request_exist($lead_id);
        if ($status == 1) {
            $created_by = $check_lead_transfer_request->created_by;
            $message_des = 'lead_transfer_request_successfully_create';
        } else if ($status == 2) {
            $created_by = $check_lead_transfer_request->updated_by;
            $message_des = 'lead_transfer_request_successfully_update';
        } else {
            return false;
        }
    ...
    public function table_lead_transfer($rel_id = "", $type = "", $action = "")
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                $basic_details = [
                    ...
                    "fathers_mobile"   => !empty($fathers_mobile)?$fathers_mobile:'',
                    "fathers_email"   => !empty($fathers_email)?$fathers_email:'',
                    "relationship_id"   => !empty($relationship_id)?$relationship_id:0,
                    "email"       => $data["email"],
                    ...
                    "dob"         => isset($dob) ? $dob : '',
                    "marital_status"         => isset($marital_status) ? $marital_status : 0,
                    "created_by"  => get_staff_user_id(),
                    "created_at"  => date('Y-m-d H:i:s')
                ];
...

    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $this->leads_model->update_lead_type($this->input->post());
        }
    }
...

    {
        ...
        if ($this->input->post()) {
            ...
            $data['custom_contact_date'] = date('Y-m-d H:i:s');
    ...
    public function add_sms($rel_id)
    {
        ...
        if ($this->input->post()) {
            ...
            $message = isset($data['lead_sms_description']) ? $data['lead_sms_description'] : $message;

            $mobile = isset($data['mobile']) ? $data['mobile'] : $mobile;



            if (isset($data['lead_sms_description'])) {

                unset($data['lead_sms_description']);
            }
            ...
            // $msg = $this->lead_sms($mobile, $message);
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, "usr=Affinityedu&key=641C3D3B-AACE-4CCD-AEE7-154CB750F1DA&to=" . $mobile . "&msg=" . $message . "&rout=Transactional&from=AFNITY");
            ...
            // $msg = true;//$this->Sms_msg91->send($phone, $message);
...

    {
        ...
        if (
            ...
            if (!$this->import->isSimulation()) {
                ...
                if ($this->import->totalImported() > 0 && $this->input->post('responsible') > 0) {
                    $staff = [
                        [
                            'staffid' => $this->input->post('responsible'),
                        ],
                    ];
                    ...
                    foreach ($staff as $member) {
                        if ($member['staffid'] != 0) {
                            $notified = add_notification([
                                'description'     => 'lead_imported_from_bulk',
                                'touserid'        => $member['staffid'],
                                'fromcompany'     => 1,
                                'fromuserid'      => get_staff_user_id(),
                                'additional_data' => serialize([
                                    $this->import->totalImported() . ' lead assigned to you.',
                                ]),
                                'link' => '#leadid=' . $lead_id,
                            ]);
                            if ($notified) {
        ...
        $data['types'] = $this->leads_model->get_type();

        // echo "<pre>";print_r($data['types']);die;
            ...
            if (is_array($ids)) {
                ...
                foreach ($ids as $id) {
                    ...
                    if ($this->input->post('mass_delete')) {
                        ...
                        // $current_status    = $this->leads_model->get_status($current_lead_data->status);
                        ...
                        if ($status || $source || $assigned || $last_contact || $visibility || $lead_type || $reference_name) {
                            ...
                            if ($status) {
                                ...
                                // $new_status_name = $this->leads_model->get_status($status)->name;

                                // $this->leads_model->log_lead_activity($id, 'not_lead_activity_status_updated', false, serialize([
...
if ($reference_name !== '') {
                                ...
                                if ($assigned > 0) {
                                    ...
                                    foreach ($staff as $member) {
                                        if ($member['staffid'] != 0) {
                                            ...
                                            $this->leads_model->log_lead_activity($id, $not_desc, $integration, $not_additional_data);
                                            ...
                                            //     send_mail_template('lead_assigned', $id, $email);
                                            //     $not_additional_data = [
    ...
    public function lead_sms()
    {

        $apiRequestUrl = 'http://sms.wishsolution.com/api/sms/SendSMS.aspx?usr=Affinityedu&key=641C3D3B-AACE-4CCD-AEE7-154CB750F1DA&smstype=TextSMS&rout=Transactional&from=AFNITY';

        $response = $this->client->request(

            'GET', //POST

            $this->apiRequestUrl . '&to=' . $number . '&msg=' . $message,
    ...
    public function update_lead_transfer_request()
    {
        ...
        if (!has_permission('leads', '', 'approval')) {
            access_denied('Lead');
        }
