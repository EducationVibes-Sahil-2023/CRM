...
class Reports extends AdminController
{
    ...
    public function leads_reports_generate()
    {
        ...
        if (!empty($_POST["view_assigned"])) {
            $index = 0;
            foreach ($_POST["view_assigned"] as $assigned) {
                $post_data = $_POST;
        ...
        $data["html_content"] = $ret;
        $this->load->view('admin/includes/head');
        $this->load->view('admin/reports/pdf', $data);
    }
        ...
        if (!empty($_POST["assigned"]) && empty($return_status)) {
            ...
            foreach ($_POST["assigned"] as $assigned) {
                ...
                if (!empty($excel_data)) {
                    ...
                    $status_summary_conversion =  array_column($status_summary_conversion, null, "index_conversion_name");
                    $status_summary_performance =  array_column($status_summary_performance, null, "index_performance_name");
                    ...
                    foreach ($source_type as $s) {
                        ...
                        foreach ($conversion_type as $ct) {
                            ...
                            $excel_array[$staff_name]["conversion_data"][$key] += $status_summary_conversion[$key]['total'];
                    ...
                    foreach ($marketing_type as $m) {
                        ...
                        foreach ($conversion_type as $ct) {
                            ...
                            $excel_array[$staff_name]["performance_data"][$key] += $status_summary_performance[$key]['total'];
                ...
                foreach ($performance_array as $key => $per) {
                    $ret .= '<div class="col-md-12 col-xs-12 border-right"><h3 class="bold">';
            ...
            $ret .= $this->lead_summary_filter(1);
        } else {
            ...
            if (!empty($excel_data)) {
                ...
                // $excel_array["Total Details"]["performance_data"] =  array_column($status_summary_performance, null, "index_performance_name");
                $status_summary_conversion =  array_column($status_summary_conversion, null, "index_conversion_name");
                $status_summary_performance =  array_column($status_summary_performance, null, "index_performance_name");
                ...
                foreach ($source_type as $s) {
                    ...
                    foreach ($conversion_type as $ct) {
                        ...
                        $excel_array["Total Details"]["conversion_data"][$key] += $status_summary_conversion[$key]['total'];
                ...
                foreach ($marketing_type as $m) {
                    ...
                    foreach ($conversion_type as $ct) {
                        ...
                        $excel_array["Total Details"]["performance_data"][$key] += $status_summary_performance[$key]['total'];
    ...
    //     // $pdf->SetFont('times', '', 8); // Font name: times, Font style: regular, Font size: 12


    //     // $pdf->AddPage();

    //     // $html = $this->load->view('admin/reports/pdf', '', true);
    //     // // $pdf->writeHTMLCell(0, 0, 0, 0, $html, 0, 1, false, true, 'J', true);
    ...
    //     //         // $ret .= '<div class="col-md-6 border-table">';
    //     //         // $ret .= '<div class="col-12 panel-body">';
    //     //         // $ret .= '<h4><b>Leads Types</b></h4><hr>';
    //     //         $ret .= '<table>';
    //     //         $ret .= '<tbody>';
    //     //         $ret .= '<tr><td><div><table><thead><tr>';

    //     //         foreach ($summary as $status) {
    //     //             $ret .= '<td><div class="col-md-3 col-xs-6 border-right"><h3 class="bold">';
    //     //             if (isset($status['percent'])) {
    //     //                 $ret .= '<span data-toggle="tooltip" data-title="' . $status['total'] . '">' . $status['percent'] . '%</span>';
    //     //             } else {
    //     //                 // Is regular status
    ...
    //     //             }
    //     //             $ret .=  '</h3>';
    //     //             $ret .= '<span style="color:' . $status['color'] . '">' . $status['name'] . '</span></div></td>';
    //     //         }
    //     //         $ret .= "</tr></thead></table></td><td><div><table><thead><tr>";
    ...
    //     //             }
    //     //             $ret .=  '</h3>';
    //     //             $ret .= '<span style="color:' . $source['color_name'] . '">' . $source['name'] . '</span></div></td>';
    //     //         }
    //     //         $ret .= '</tr></thead></table></td></tr>';
    //     //         $ret .= '</tbody>';
    //     //         $ret .= '</table>';
    //     //         // $ret .= '</div>';
    //     //         // $ret .= '</div>';
    ...
    //     // } else {
    //     // }
    //     // // $data["html_content"] = htmlspecialchars($ret);
    ...
    //     // $html = $this->load->view('admin/reports/pdf');
    //     // // echo $html = $this->load->view('admin/reports/pdf', $data);
    //     // die;
    //     // // $html = $ret;

    //     // // $pdf->writeHTMLCell(0, 0, 0, 0, $ret, 0, 1, false, true, 'J', true);
