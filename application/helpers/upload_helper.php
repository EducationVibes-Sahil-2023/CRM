...
function handle_client_attachments_upload_docs($id, $customer_upload = false, $doctype)
{
    ...
    if (isset($_FILES[$doctype]['name'])
        ...
        if (!is_array($_FILES[$doctype]['name'])) {
            ...
            $_FILES[$doctype]['type']     = [$_FILES[$doctype]['type']];
            $_FILES[$doctype]['tmp_name'] = [$_FILES[$doctype]['tmp_name']];
            $_FILES[$doctype]['error']    = [$_FILES[$doctype]['error']];
            $_FILES[$doctype]['size']     = [$_FILES[$doctype]['size']];
        }

        _file_attachments_index_fix($doctype);
        for ($i = 0; $i < count($_FILES[$doctype]['name']); $i++) {
            hooks()->do_action('before_upload_client_attachment', $id);
            // Get the temp file path
            $tmpFilePath = $_FILES[$doctype]['tmp_name'][$i];
            // Make sure we have a filepath
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                _maybe_create_upload_path($path);
                $filename    = unique_filename($path, $_FILES[$doctype]['name'][$i]);
                $newFilePath = $path . $filename;
                // Upload the file into the temp dir
                // print_r($newFilePath);die;
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    $attachment   = [];
                    $attachment[] = [
                    'file_name' => $filename,
