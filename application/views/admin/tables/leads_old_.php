...
function calculateOfficeTimeDiff(
    ...
    return sprintf('%d D %02d:%02d', $days, $hours, $minutes);
}
...
if (!empty($this->ci->input->post('up_to_date'))) {
    ...
    $condition = $this->ci->input->post('time_condition');
    $minutes   = $this->ci->input->post('time');

    if (!empty($condition) && is_numeric($minutes)) {

        // allow only valid operators
        ...
        THEN CONCAT(
        DATE_ADD(DATE(tblleads.dateassigned), INTERVAL 1 DAY),
        ' ',
        COALESCE(tblstaff_department.office_start_time,'10:00:00')
        )

        WHEN TIME(tblleads.dateassigned) >
        COALESCE(tblstaff_department.office_end_time,'20:00:00')
        ...
        WHERE c.contact = tblleads.phonenumber
        AND (c.call_start + 19800) > UNIX_TIMESTAMP(tblleads.dateassigned)
        AND c.staffid = tblleads.assigned
        )
        )
...
// if(is_admin()){
//  $where[]   = "AND mass_assigned_status !=1 ";
// $where[] = " AND $sTable.dateadded > '2026-01-01 00:00:00' ";
...
// "CASE
//     /* get first call timestamp */
//     WHEN (
//         SELECT MIN(call_start)
//         FROM tblcalls_activity_logs c
//         WHERE c.contact = tblleads.phonenumber
//         AND (c.call_start + 19800) > UNIX_TIMESTAMP(tblleads.dateassigned)
//         AND c.staffid = tblleads.assigned
...
/* ✅ CASE 5: WORKING DAY → CALCULATE HOURS */
ELSE
(
    SELECT CONCAT(
        FLOOR(GREATEST(diff_sec,0) / 86400), ' D ',
        LPAD(FLOOR((GREATEST(diff_sec,0) % 86400) / 3600), 2, '0'), ':',
        LPAD(FLOOR((GREATEST(diff_sec,0) % 3600) / 60), 2, '0'), ':',
        LPAD(GREATEST(diff_sec,0) % 60, 2, '0')
    )
    FROM (
        SELECT TIMESTAMPDIFF(
            SECOND,

            /* ✅ START (clamp to office start) */
            GREATEST(
                tblleads.dateassigned,
                CONCAT(
                    DATE(tblleads.dateassigned),
...
END AS first_connect_difference",
"tblstaff_department.office_start_time as office_start_time",
"tblstaff_department.office_end_time as office_end_time",

"(SELECT MIN(c.call_start)
    FROM tblcalls_activity_logs c
    WHERE c.contact = tblleads.phonenumber
    AND (c.call_start) > UNIX_TIMESTAMP(tblleads.dateassigned)
    AND c.staffid = tblleads.assigned
) AS phonenumber_duration",

"(SELECT MIN(c.call_start)
    FROM tblcalls_activity_logs c
    WHERE c.contact = tblleads.alternative_phonenumber
    AND (c.call_start) > UNIX_TIMESTAMP(tblleads.dateassigned)
    AND c.staffid = tblleads.assigned
) AS alternative_duration"
]);


$join[] = " JOIN tblstaff on tblstaff.staffid = tblleads.assigned ";

$join[] = " LEFT JOIN tblstaff_department on tblstaff_department.id = tblstaff.department ";
...
// if(!empty($this->ci->input->post('time_condition')) && !empty($this->ci->input->post('time')) && $this->ci->input->post('time') >0)
// {
//      $where[] = " AND {$call_table}.adjusted_call_start >= '{$up_from_date} 00:00:00'
//         AND {$call_table}.adjusted_call_start <= '{$up_to_date} 23:59:59' ";
// }
...
$aditional_order_by = " Order By CASE WHEN tblleads.status = 33 THEN 0 ELSE 1 END ";


    $result = data_tables_init_($aColumns, $sIndexColumn, $sTable, $join, $where, $additionalColumns, $group_by, '', '', $search_column,$aditional_order_by);
...
//   $row[] = $aRow['first_connect_difference'];
    ...
    foreach ($custom_fields as $key => $field) {
        $row[] = !empty($customFieldValues[$aRow[str_replace(" ", "_", strtolower($field['name']))]]) ? mb_substr($customFieldValues[$aRow[str_replace(" ", "_", strtolower($field['name']))]] , 0, 30): '';
    }
