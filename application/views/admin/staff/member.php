...
<div id="wrapper">
                        ...
                        <div class="form-group select-placeholder">
                           ...
                           <select name="reporting_person" data-live-search="true" id="reporting_person" class="form-control selectpicker" data-none-selected-text="Select Reporting Person">
                              ...
                              <?php foreach ($staff as $s) {
                                 $selected = '';
                                 if (isset($member)) {
                                    if ($member->reporting_person == $s['staffid']) {
                                       $selected = 'selected';
                                    }
                                 }
                              ?>
                                 <option value="<?php echo $s['staffid']; ?>" <?php echo $selected; ?>><?php echo ucfirst($s['firstname']) . " " . ucfirst($s['lastname']); ?></option>
                              <?php } ?>
                           </select>
                        </div>
                        ...
                        <div class="form-group select-placeholder">
                           ...
                           <select name="lead_type" data-live-search="true" id="lead_type" class="form-control selectpicker" data-none-selected-text="Select Lead Type">
                              ...
                              if (!empty($lead_type)) {
                                 ...
                                 foreach ($lead_type as $lead) {
                                    $selected = '';
                                    if (isset($select_lead_type)) {
                                       if (in_array($lead['id'], $select_lead_type)) {
                                          $selected = 'selected';
                                       }
                                    }
                              ?>
                                    <option value="<?php echo $lead['id']; ?>" <?php echo $selected; ?>><?php echo $lead['name'] ?></option>
                              <?php }
                              } ?>
                           </select>
                        </div>
        ...
        <?php } ?>
                        ...
                        <div class="form-group select-placeholder assign_state_div" style="display:<?= !empty($member->department_head) ? 'none' : '' ?>">
                           ...
                           <!-- <select name="assign_state[]" data-live-search="true" multiple id="assign_state" class="form-control selectpicker" data-none-selected-text="Select State">
                              ...
                              <?php
                              if (!empty($state_list)) {
                                 $select_state_array = explode(',', $member->assign_state);
                           ...
                           echo render_select('assign_state[]', $state_list, array('id', 'name'), '', $select_state_array, array('data-width' => '100%', 'data-none-selected-text' => 'Select State', 'multiple' => true, 'data-actions-box' => true), array(), 'no-mbot', '', false, "assign_state");
                           echo '</div>';
                           ?>
                        </div>
   ...
   <script>
      ...
      function set_department_head(obj) {
         $("#assign_state").val(null).selectpicker("refresh")
         $(".assign_state_div").find("select").val('');
         $("#facebook_lead_name").val('');
         if ($(obj).prop('checked') == true) {
            $(".assign_state_div").hide();
            $("#facebook_lead_name").parent("div").hide();

         } else {
            $(".assign_state_div").show();
            $("#facebook_lead_name").parent("div").show();
