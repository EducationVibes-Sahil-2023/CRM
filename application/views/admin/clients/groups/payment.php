...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        var tAPI = "";
        $(function() {
            tAPI = initDataTable('.table-payment-table', admin_url + 'clients/payment_table/' + <?= $client_id ?>);
        });
    ...
    async function showSplit($select, jsonData, payment_id) {
        ...
        nestedTableIdArray[nestedTableId] = await initDataTable(
            `#${nestedTableId}`,
            admin_url + 'clients/payment_table/' + <?= $client_id ?>,
...
";
    ...
    if (!empty($client_id)) {
        ...
        $university_shortlisting = $ci->clients_model->university_shortlisting($client_id, 1);
    ...
    if (!empty($payment_id)) {
        $applicant_payment_data = $ci->quotation_model->applicant_payment_data($client_id, $payment_id);
                        ...
                        <div class="col-lg-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="acadmic_year" id="acadmic_year" readonly
                                    value="<?= htmlspecialchars($applicant_payment_data->acadmic_year ?? $acadmic_year) ?>">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <?php
                                $year_options = array_map(function ($year) {
                                    return ['id' => $year, 'name' => $year . ' Year'];
                                }, $years_array);
        ...
        function vendor_update(obj, modeId) {
            ...
            if (modeId == 1 || modeId == 4 || modeId == 6) {
                ...
                vendor_select.append(
                    '<option value="<?= htmlspecialchars($primary_university) ?>" selected>' +
                    '<?= htmlspecialchars($primary_university) ?>' +
                    '</option>'
                );

                vendor_select.val("<?= htmlspecialchars($primary_university) ?>");
                vendor_select.selectpicker('refresh');
        ...
        async function document_approved(obj, status = 0, quotation_payment_id, doc_id = null) {
            ...
            try {
                ...
                let response = await $.ajax({
                    url: "<?= base_url("admin/clients/quotation_payment_approved") ?>",
                    method: "POST",
                    data: upload_data,
                    contentType: false,
                    processData: false,
