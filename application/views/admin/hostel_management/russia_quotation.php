...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        $(function() {
            ...
            tAPI = initDataTable('.table-quotation-table', admin_url + 'hostel_management/quotation_table/russia/' + <?= $getId ?>);
...
$rentalInfo = $hostelRentelData[$hostelData->hostel ?? '']["rental_details"] ?? [];

if (!empty($rentalInfo)) {
    $rentalInfo = json_decode($rentalInfo, true);
    $rentalInfo = array_column($rentalInfo, null, 'fee_id');
}
                        ...
                        <?php if (!empty($university_applicant_fees) && !empty($get_currencies)) :  ?>
                            <div id="applicant_fees">
                                <div class="row align-items-center mb-2">
                                    <div class="col-md-6">
                                        <h4 class="mb-0">Food/Mess,Medical,OTC and Accommodation</h4>
                                    </div>
    ...
    function get_hostel_rentInfo(year) {
        return new Promise((resolve, reject) => {
            try {
                ...
                let hostel_id = $("select[name='hostel']").val();
                let get_academic_year = $("select[name='acadmic_year']").val();
                ...
                let rental_details = rental_details_json ? JSON.parse(rental_details_json) : {};
