...
<script>
    function getPayementInformation(hostel_info_id) {
        ...
        if ($container.is(":visible")) {
            ...
            $.ajax({
                url: admin_url + "hostel_management/payment_information/russia", // CI controller method
                type: "POST",
                data: {
                    ...
                    <div class="panel_s shadow">
                        <div class="panel-body">
                            ...
                            <table class="table table-bordered" id="exchangeTable">
                                <thead>
                                    <tr>
                                        <th style="width: 40%; text-align: center;">Creadit Currency</th>
                                ...
                                <tbody id="exchangeTableBody">
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" step="0.01" class="form-control currency-amount"
                                                    oninput="calculateInrValue()" required name="exchange_value[]" placeholder="0.00">
                                            </td>
                                            <td class="text-center">
                            ...
                            <div id="applicant_fees">
                                ...
                                <div class="payment_payment">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-2 form-group split-type-dropdown"
                                            style="display:<?=
                                                            (!empty($applicant_payment_data->payment_type) &&
                                                                ($applicant_payment_data->payment_type == PACKAGE_FEES_ID ||
                                                                    $applicant_payment_data->payment_type == RETURN_FEES_ID))
                                                                ? 'block' : 'none'
                                                            ?>">
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                        <tr data-fee-id="<?= $split['fee_id'] ?>">
                                                                ...
                                                                <div class="input-group">
                                                                    ...
                                                                    <div class="input-group-addon">
                                                                        <select name="amount_currency_type[<?= $split['fee_id'] ?>]"
                                                                            ...
                                                                            <?php foreach ($get_currencies as $c): ?>
                                                                                <option value="<?= $c['id'] ?>" <?= $c['id'] == $split['credit_currency'] ? 'selected' : '' ?> data-symbol="<?= htmlspecialchars($c['symbol']) ?>">
                                                                                    <?= htmlspecialchars($c['name']) ?>
                                                                                </option>
                                                                            <?php endforeach; ?>
                                                                        </select>
                                                            ...
                                                            <td>
                                                                <select disabled name="fee_currency[<?= $split['fee_id'] ?>]" class="form-control document_currency">
                                                                    <?php foreach ($get_currencies as $c): ?>
                                                                        <option value="<?= $c['id'] ?>" <?= $c['id'] == $split['document_currency'] ? 'selected' : '' ?> data-symbol="<?= htmlspecialchars($c['symbol']) ?>">
                                                                            <?= htmlspecialchars($c['name']) ?>
                                                                        </option>
                                                                    <?php endforeach; ?>
                                                                </select>
...
var hostel_vendors = <?= json_encode($hostelVendors) ?>;
        ...
        function get_hostel_rentInfo(hostel_id, year) {
            return new Promise((resolve, reject) => {
                try {
                    console.log("Requested Year:", year);

                    let get_academic_year = $("input[name='acadmic_year']").val();
                    let key = get_academic_year + '_' + year;

                    console.log("Computed Key:", key);

                    // Validate required data
                    if (!get_university_rentData || !get_university_rentData[hostel_id]) {
                        reject(`No rental data found for hostel: ${hostel_id}`);
                        return;
                    }

                    // Parse rental details safely
                    let rental_details_json = get_university_rentData[hostel_id]?.rental_details;
                    let rental_details = {};

                    console.log("Raw rental_details_json:", rental_details_json);
                    console.log("Hostel ID:", hostel_id);

                    if (rental_details_json) {
                        try {
                            // Parse only if it's a string
                            const parsed = typeof rental_details_json === "string" ?
                                JSON.parse(rental_details_json) :
                                rental_details_json;

                            // Ensure parsed[key] exists and is an array
                            if (parsed[key] && Array.isArray(parsed[key])) {
                                rental_details = Object.fromEntries(parsed[key].map(item => [item.fee_id, item]));
                            } else {
                                console.warn(`⚠️ rental_details_json[${key}] not found or not an array`, parsed);
                                rental_details = {};
                            }

                        } catch (error) {
                            console.error("❌ Failed to parse rental_details_json:", error);
                            rental_details = {};
                        }
                    }
                    ...
                    let selectedData = rental_details || [];
        ...
        $(function() {
            ...
            // if (year != "") {
            //     // let hostelId = hostel_quotations[year].hostel;
            //     get_hostel_rentInfo(year);
            // }
        });
        ...
        function check_quotations(id) {
            try {
                ...
                if (!selected_quotation) {
                    console.warn("No quotation found for ID:", id);
                    return;
                }
                ...
                const acadmic_year = selected_quotation.acadmic_year || "";
                const year = selected_quotation.year || "";

                console.log("Selected Quotation:", selected_quotation);
                ...
                const $acadmicYearInput = $("input[name='acadmic_year']");
                ...
                if ($acadmicYearInput.length) $acadmicYearInput.val(acadmic_year);
                if ($yearInput.length) $yearInput.val(year);
