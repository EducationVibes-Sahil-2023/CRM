            ...
            <div id="msform">
                ...
                ?>
                    <fieldset id="<?= !empty($track["show_div_name"]) ? $track["show_div_name"] : '12' ?>" style="display:<?= ($applicant_status == $k) ? "show" : "none" ?>">
                                ...
                                <div class="add_university_div_block">
                                    ...
                                    ?>
                                            <div class="col-md-12 university_div  <?= ($short_list["university_status"] == 1) ? '' : 'university_div_'; ?>   <?= ($short_list["university_status"] == 1) ? 'bg-success' : (($short_list["university_status"] == 2) ? 'bg-danger' : 'bg-warning') ?>" style="    padding: 20px; margin: 4px;">
                                                ...
                                                <div class="col-md-3 <?= ($short_list["university_status"] == 1) ? 'check_university' : 'not_check_university' ?> ">
                                                    <?php
                            ...
                            <div class="offer_div">
                                ...
                                ?>
                                        <div class="col-md-12 university_div_application mt-2 <?= $short_list["fee_status"] == 1 ? "bg-success pre-select-offer" : '' ?>">
                                            <div class="col-md-1">
                                                <input type="radio" name="select_university" value="<?= $short_list["id"] ?>" <?= $short_list["fee_status"] == 1 ? "checked" : '' ?>>
                                            </div>
                                            <div class="col-md-2">
                                                <input type="hidden" name="university_id" value="<?= $short_list["id"] ?>">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <?php if (!empty($short_list['media_file'])) { ?>
                                                    <div class="row media-text-div-offer">
                                                        ...
                                                        <div class="col-md-2">
                                                            <?php if (!empty($short_list["media_file"])) { ?>
                                                                <a class="col-md-12 download_document" accept="image/*,application/pdf" onclick="window.open(`<?= base_url($short_list['media_file']) ?>`, '_blank');" href="javascript:void(0);" type="button"><i class="fa fa-download" aria-hidden="true"></i></a>
                                                            <?php }
                                                            ?>
                                                        </div>
                                ...
                                <?php if (!empty($acceptance_text)) { ?>
                                    <div class="acceptance_text_approval_message_action ">

                                        <p class="p-3 text-warning"><?= $acceptance_text ?></p>
                                    </div>
                                <?php } ?>
                        ...
                        <?php } else if ($track["show_div_name"] == "fees_div") { ?>
                            <div class="fees_div">
                                ...
                                <h4 class="text-success">
                                    Your payment has been successfully deposited under the <?= !empty($selected_university_name) ? $selected_university_name : '' ?>
                                </h4>
                            </div>
                        <?php } ?>
                        <?php if ($k > 0) { ?>
                        ...
                        ?>
                            <button class="btn btn-primary action-button action-button-update" onclick="handleSubmit(event)">Update</button>
                        <?php
                        }
                        ?>
                        <?php
                ...
                <div class="col-md-2">
                    ...
                    $(document).ready(function() {
                        ...
                        if ($(".pre-select-offer").length > 0) {
                            ...
                            $(this).parents(".university_div_application").find(".hide-fee-div").find(".hide-fee-div").prop('disabled', true).selectpicker('refresh');
                            $("#offer_div").find(".hide-fee-div").find("button").attr("disabled", true);
                        }
                    })
                    $("#offer_div").find("input[name='select_university']").click(function() {
                        $("#offer_div .university_div_application").removeClass("bg-success");
                        $(this).parents(".university_div_application").addClass("bg-success");
                        $(".hide-fee-div").hide();
                        $(this).parents(".university_div_application").find(".hide-fee-div").find("input[type='file']").val("");
                        $(this).parents(".university_div_application").find(".hide-fee-div").show();
                        $(this).parents(".university_div_application").find(".hide-fee-div").find('input, select').prop('disabled', false).selectpicker('refresh');
                    })

                    async function fee_upload(obj) {
                        let fileInput = $(obj).closest(".university_div_application").find(".hide-fee-div").find("input[type='file']").prop("files")[0];
                        let university_id = $("input[name='select_university']:checked").val();

                        if (!fileInput) {
                            alert_float("success", "Select Fees file.");
                            return false;
                        }
                        ...
                        formData.append("media_file", fileInput);

                        try {
                            let response = await fetch("<?= base_url("clients/university_fee_update") ?>", {
                                method: "POST",
                                body: formData
                            });
