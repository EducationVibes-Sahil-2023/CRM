...
<div class="row">
    <div class="col-md-12">
        <div class="form-container">
            <h4 class="fs-title">Visa Documents <?php if (!empty($country_name)) { ?><span>(<?= $country_name ?>)</span> <?php } ?> <button class="btn btn-primary pull-right add-document add-university-btn" type="button" style="display: none1;" onclick="add_documents()"><i class="fa fa-plus" aria-hidden="true"></i></button>
            </h4>
            <hr>
            ...
            <div class="bio-interview-date">
                <div class="form-group row">
                    ...
                    <div class="col-md-6" style="display:<?= !empty($visa_data[0]["biometric_status"]) ? '' : 'none' ?>">
                        <input type="text" id="biometric-date" class="form-control datepicker " disabled value='<?= !empty($visa_data[0]["biometric_date"]) ? $visa_data[0]["biometric_date"] : '' ?>' autocomplete="off" placeholder="Biometric Date">
                    </div>
                </div>
                <div class="form-group row mt-5">
                    ...
                    <div class="col-md-6" style="display:<?= !empty($visa_data[0]["interview_status"]) ? '' : 'none' ?>">
                        <input type="text" id="interview-date"  disabled value='<?= !empty($visa_data[0]["interview_date"]) ? $visa_data[0]["interview_date"] : '' ?>' class="form-control datepicker" autocomplete="off" placeholder="Interview Date">
                    </div>
                </div>
...
console.log(client_id);
    $("input[type=checkbox],select,textarea").attr("disabled", true);
    ...
    function upload_document(type) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                let response = await $.ajax({
                    url: "<?= base_url("clients/upload_visa_documents") ?>",
                    method: "POST",
                    data: upload_data,
                    contentType: false,
                    processData: false
