...
function setfilter_fixed() {
    ...
    if (typeof window.initialTop === 'undefined' || window.initialTop === null) {
        const rect = stickyFilter.getBoundingClientRect();
        window.initialTop = rect.top + window.scrollY; // Get absolute top position
    }

    const scrollTop = window.scrollY || document.documentElement.scrollTop;

    if (scrollTop >= window.initialTop) {
