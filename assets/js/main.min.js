...
function applicant_mark_as(status, userId, isCanceled = "", isRefunded = "") {
    ...
    $(".applicant_status_modal_div").hide(); // Hide all modal divs
    ...
    // If the status requires a form reset, reset the form and hide the modal
    if (isCanceled == 1 || isRefunded == 1) {
        $("#applicant_status_change_form")[0].reset(); // Proper form reset
        $("#applicant_status_change").modal("show"); // Show modal instead of hiding
        setTimeout(function () {
    ...
    // Assign values for AJAX request
    data.status = status;
    data.userid = userId;
    data.append(csrfData.token_name, csrfData.hash);

    // AJAX request to update client status
    $.post(admin_url + "clients/update_client_status", data)
        .done(function (response) {
            applicant_reload(); // Reload the applicant list after status update
        })
        .fail(function (xhr, status, error) {
            console.error("Error updating status:", error); // Log errors in case of failure
        });
}
