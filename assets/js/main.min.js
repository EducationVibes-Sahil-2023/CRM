var html_append = `<div class="loading-upper" style="display:;">
<div class="loading"></div>
</div>`;
$("#wrapper").prepend(html_append);
...
var original_top_search_val, table_leads,table_leads_new, table_activity_log, table_estimates, table_invoices, table_tasks, menu_href_selector, tab_active = get_url_param("tab"),
...
function initDataTable(e, t, a, i, n, s, s_status = true) {
    if (t.includes("leads/table")) {
        // s_status = false;
    }
    ...
    var p = [5, 10, 25, 50, 100, 500],
        _ = [5, 10, 25, 50, 100, 500];

    if (TablePagination != undefined && TablePagination != "") {
        const paginationOptions = [5, 10, 25, 50, 100, TablePagination];
        p = paginationOptions;
        _ = paginationOptions;
    }
    ...
    var m = {
        ...
        ajax: {
            ...
            type: "POST",
            data: function (e) {
                // Ensure csrfData is defined and has required properties
                if (typeof csrfData !== "undefined" && csrfData.token_name && csrfData.hash) {
                    e[csrfData.token_name] = csrfData.hash; // Add CSRF token to the object
                }

                // Iterate over the `n` array or object to populate `e`
                if (n && typeof n === "object") {
                    Object.keys(n).forEach(key => {
                        if (typeof n[key] === "string") {
                            // Assign values from `n` to `e` for string keys
                            e[key] = $(n[key]).val();
                        } else if (typeof n[key] === "object" && n[key] !== null) {
                            // Ensure `e[key]` is initialized as an object
                            e[key] = e[key] || {};
                            e[key] = JSON.stringify(n[key]); // Populate nested values
                        }
                    });

                    // Debugging output
                    // // console.log("Populated Object e:", e);
                }

                // Add last order identifier if it exists
...
var ajax_timer, ajax_search_tags;

function init_tags_inputs() {
    appTagsInput_();
    $(".tagit-new").find("input").on("keyup", function () {


        clearTimeout(ajax_timer);
        if (ajax_search_tags) {
            ajax_search_tags.abort();
        }
        // Sets new ajax_timer that may or may not get cleared
        ajax_timer = setTimeout(() => {
            appTagsInput_('', $(this).val());
        }, 2000);
    });
}

function get_tags_ajax(search) {
    return new Promise((resolve, reject) => {
        ajax_search_tags = $.ajax({
            url: admin_url + "leads/get_tags",
            dataType: "json",
            method: "POST",
            data: { search: search },
...
async function appTagsInput_(e, search) {
    if (typeof e === 'undefined' || e == '') {
        e = $("body").find("input.tagsinput");
    }

    var app_data = {
        available_tags: [], // Example tags, replace with actual tags
        available_tags_ids: [], // Corresponding tag IDs, replace with actual IDs
        lang: {
            tag: "Add a tag" // Example placeholder text, replace with actual text
        }
    };

    if (search != undefined && search != "") {
        try {
            const data = await get_tags_ajax(search);
            app_data.available_tags = data.available_tags || [];
            app_data.available_tags_ids = data.available_tags_ids || [];
        } catch (error) {
    ...
    if (e.length) {
        e.tagit({
            availableTags: app_data.available_tags,
            allowSpaces: true,
            animate: true,
            placeholderText: app_data.lang.tag,
            ...
            autocomplete: {
                ...
                source: async function (request, response) {
                    try {
                        const data = await get_tags_ajax(request.term);
                        response(data.available_tags);
                    } catch (error) {
                        console.error("Error fetching tags:", error);
                        response([]);
                    }
            ...
            afterTagAdded: function (event, ui) {
                var tagText = $.trim($(ui.tag).find(".tagit-label").text());
                var tagIndex = app_data.available_tags.indexOf(tagText);
                if (tagIndex !== -1) {
                    var tagId = app_data.available_tags_ids[tagIndex];
                    $(ui.tag).addClass("tag-id-" + tagId);
                }
...
// function init_roles_permissions(roleId, triggerChange = true) {

//     // ✅ Get roleId if not passed
//     roleId = roleId === undefined ? $('select[name="role"]').val() : roleId;

//     // ✅ Skip conditions
//     if ($('input[name="administrator"]').prop("checked") || !roleId) return;

//     let $rows = $("table.roles tr");

//     // ✅ Cache rows by data-name (BIG performance boost)
//     let rowMap = {};
//     $rows.each(function () {
...
//     let request = $.ajax({
//         url: admin_url+"staff/role_changed/" + roleId,
//         method: "GET",
//         dataType: "json",
//         timeout: 5000
//     });
...
//         // ✅ Reset once
//         $rows.find(".capability")
//             .not('[data-not-applicable="true"]')
//             .prop("checked", false);

//         // ✅ Loop response only
//         Object.keys(response).forEach(roleName => {

//             let permissions = response[roleName];
//             let $row = rowMap[roleName]; // ✅ use cached row

//             if (!$row) return;

//             permissions.forEach(permission => {

//                 let $input = $row.find(`input[id="${roleName}_${permission}"]`);
...
//                     if (triggerChange) {
//                         if (permission === "view") {
//                             $row.find("[data-can-view]").trigger("change");
//                         } else if (permission === "view_own") {
//                             $row.find("[data-can-view-own]").trigger("change");
//                         }
//                     }
//                 }
//             });
...
function check_tt_copy(obj) {
    ...
    if (modeId == 2 || (modeId == 1 && transaction_type == 1)) {
        paymentSection.find("input.tt_proof ").attr("disabled", false);

        let hasEyeIcon = paymentSection.find("input.tt_proof ")
        ...
        if (hasEyeIcon) {
            ...
            paymentSection.find("input.tt_proof ").attr("required", true);
        }
    }
    else {
...
if($("select.mode").val() == 5)
{
if ($("select.transaction_type").filter(':visible:not(:disabled)').length > 0) {
    $("select.transaction_type").val(2).selectpicker('refresh');
}
}
else
{
    //  $("select.transaction_type").val('').selectpicker('refresh');
}

    if (typeof CheckPackageCondition === 'function') {
...
function acceptText(el, type = "") {
    ...
    if (controlKeys.includes(char)) {
        return true; // Always allow editing/navigation keys
    }

    let regex;

    switch (type.toLowerCase()) {
        case "text":   // ✅ Allow only letters and spaces
            regex = /^[A-Za-z\s]$/;
            break;

        case "number": // ✅ Allow only digits
            regex = /^[0-9]$/;
            break;

        case "email":  // ✅ Allow email-friendly chars
            regex = /^[A-Za-z0-9@._-]$/;
            break;

        default:       // ✅ If no type provided → allow all
            return true;
    }
...
// function init_lead(e, t,isTransfer=false) {
//     $("#task-modal").is(":visible") && $("#task-modal").modal("hide"), init_lead_modal_data(e, void 0, t) && $("#lead-modal").modal("show") if (isTransfer) {
//         setTimeout(() => {
//             $("#show_transfer_lead_div").trigger("click");
//         }, 200);
...
function init_lead(e, t='', isTransfer = '', visitorStatus = 0,leadtabopen="") {

console.log(leadtabopen);
    // Hide task modal if it is visible
    ...
    if (isTransfer != '') {
        const intervalId = setInterval(() => {
            if ($("#lead-modal").is(":visible")) {
                $(isTransfer).trigger("click");
                
                // // console.log(isTransfer);
                // // console.log(visitorStatus);
                if (visitorStatus == 1) {

                    if (is_admin == '1' || (role_type !== undefined && role_type == 3)) {
                        setTimeout(() => {
                            $(".horizontal-tabs ul.nav-tabs-horizontal li").not(".active").removeClass("disabled").css("pointer-events", "unset");
                        }, 500);
    ...
    // Initialize lead modal data and show lead modal
    if (init_lead_modal_data(e, void 0, t, visitorStatus)) {
    ...
    // console.log("skip 1");
    setTimeout(function () {
        // console.log($("#lead-modal").is(":visible"));
      if ($("#lead-modal").is(":visible")) {
        //   console.log("skip 2");
                    // console.log(leadtabopen);
                    
                document.querySelector('a[href="#'+leadtabopen+'"]').click();
    ...
    // Trigger click on the transfer lead div if isTransfer is true
...
function validate_lead_convert_to_client_form() {
    ...
    // Loop through inputs in #applicant_fees
    $("#applicant_fees input.currency-amount").each(function () {
        const value = $(this).val(); // Get the value of the input
        const isRequired = $(this).attr("required") !== undefined; // Check if the 'required' attribute exists
        ...
        // Debugging (optional)
        // // console.log("Value:", value, "Required:", isRequired, "Name:", name);
    });
    ...
    var e = {
        ...
        password: {
            required: {
                depends: function (e) {
                    if (!$('input[name="send_set_password_email"]').prop("checked")) return true;
                }
            }
        },
        email: {
    ...
    // Merge predefined fields and additional fields
    if (Object.keys(additional_fields).length > 0) {
        e = Object.assign(e, additional_fields);
    }

    // Add company validation if required
    if (1 == app.options.company_is_required) {
        e.company = "required";
    }
...
function lead_profile_form_handler(e) {
    ...
    return $(".lead-save-btn").addClass("disabled"), $.post(e.attr("action"), t).done(function (e) {
        $(".lead-save-btn").removeClass("disabled");
        "" !== (e = JSON.parse(e)).message && alert_float(e.success == true ? "success" : "danger", e.message), e.proposal_warning && 0 != e.proposal_warning ? ($("body").find("#lead_proposal_warning").removeClass("hide"), $("body").find("#lead-modal").animate({
            scrollTop: 0
...
function _lead_init_data(e, i) {
    ...
    if (-1 < ["#tab_lead_profile", "#attachments", "#lead_notes", "#lead_activity", "#gdpr"].indexOf(t) && (window.location.hash = t), initDataTableInline($("#consentHistoryTable")), $("#lead-modal").find(".gpicker").googleDrivePicker({
        ...
        "undefined" != typeof Dropbox && document.getElementById("dropbox-chooser-lead").appendChild(Dropbox.createChooseButton({
            ...
            success: function (e, t) {
                t = JSON.parse(t), 0 === sthis.getUploadingFiles().length && 0 === this.getQueuedFiles().length && _lead_init_data(t, t.id)
            }
...
function init_lead_modal_data(t, e, a, visitorStatus) {
    var i = (e !== undefined ? e : "leads/lead/") +
        (t !== undefined ? t : "") +
        (visitorStatus === 1 ? ("/" + visitorStatus) : "");
    if (!0 === a) {
        var n = "?"; - 1 < i.indexOf("?") && (n += "&"), i += n + "edit=true&visitorStatus=" + visitorStatus
    }
...
function lead_mark_as_type(e, t) {
    var a = {};
    a.type = e, a.leadid = t, $.post(admin_url + "leads/update_lead_type", a).done(function (e) {
        table_leads.DataTable().ajax.reload(null, !1)
    })
}

function department_mark_as(e, t) {
    var a = {};
    a.department = e, a.staffid = t, $.post(admin_url + "staff/update_department_type", a).done(function (e) {
        staff_data_table.ajax.reload(null, !1)
    })
}

function applicant_mark_as(status, userId, isCanceled = "", isRefunded = "") {
    let data = new FormData(); // Use FormData instead of plain object
    $(".applicant_status_modal_div").hide();

    $("#applicant_status_change_form input[name='userid']").val(userId);
    $("#applicant_status_change_form input[name='status']").val(status);
    ...
    if (isCanceled == 1 || isRefunded == 1) {
        $("#applicant_status_change_form")[0].reset();
        $("#applicant_status_change").modal("show");
        setTimeout(function () {
            applicant_status_change(1);
        }, 500);
        return false;
    }
    ...
    data.append(csrfData.token_name, csrfData.hash);

    $.ajax({
        url: admin_url + "clients/update_client_status",
        method: "POST",
        data: data,
        processData: false,
        contentType: false,
...
function application_mark_as(status, client_id, shortlisting_id, isCanceled = "", isRefunded = "") {
    let data = new FormData(); // Use FormData instead of plain object
    $(".applicant_status_modal_div").hide();

    $("#applicant_status_change_form input[name='shortlisting_id']").val(shortlisting_id);
    $("#applicant_status_change_form input[name='status']").val(status);

    if (isCanceled == 1) {
        $(".canceled_div_remark").show();
    }
    if (isRefunded == 1) {
        $(".refund_div_remark").show();
    }
    ...
    data.append('status', status);
    data.append('shortlisting_id', shortlisting_id);
    data.append('client_id', client_id);
    data.append(csrfData.token_name, csrfData.hash);

    $.ajax({
        url: admin_url + "clients/update_application_status",
        method: "POST",
        data: data,
        processData: false,
        contentType: false,
...
//             t_mass = $("#mass_re-assignation").prop("checked"),
//             a = [],
//             i = {};
...
//         }
//         else {
//             if (t_mass == 1 || void 1 === t_mass) {
//                 i.mass_assign = 1;
//                 i.mass_assigned = $("#mass_assigned").val();
//             }
//             if (i.lost = $("#leads_bulk_mark_lost").prop("checked"), i.status = $("#move_to_status_leads_bulk").val(), i.assigned = $("#assign_to_leads_bulk").val(), i.source = $("#move_to_source_leads_bulk").val(), i.leadtype = $("#leadtype").val(), i.last_contact = $("#leads_bulk_last_contact").val(), i.tags = $("#tags_bulk").tagit("assignedTags"), i.visibility = $('input[name="leads_bulk_visibility"]:checked').val(), i.assigned = void 0 === i.assigned ? "" : i.assigned, i.visibility = void 0 === i.visibility ? "" : i.visibility, "" === i.status && !1 === i.lost && "" === i.assigned && "" === i.source && "" === i.leadtype && "" === i.last_contact && 0 == i.tags.length && "" === i.visibility) return
...
//                 if (table_leads != undefined) {
//                     table_leads.DataTable().ajax.reload(null, !1);
//                 }
//                 $("#leads_bulk_actions").modal("hide"), alert_float("success", "Bulk action performe successfully")
//                 // window.location.reload()
//             }).fail(function (e) {
//                 $(e).removeClass("disabled");
//                 $("#leads_bulk_actions").modal("hide"), alert_float("danger", e.responseText)
...
function leads_bulk_action(e) {
    if (confirm_delete()) {
        var t = $("#mass_delete").prop("checked"),
            t_mass = $("#mass_re-assignation").prop("checked"),
            d_date = $("#delete_created_date").prop("checked"),
            a = [],
            i = {};
        if (t == 1 || t_mass == 1) {
            if (t == 1) {
                ...
                i.leadtype = $("#leadtype").val();
                i.reference_name = $("#bulk_reference_name").val();
                if (d_date == 1) {
                    i.delete_created = 1;
                }
            }
            ...
            i.assigned = typeof i.assigned === "undefined" ? "" : i.assigned;
            i.visibility = typeof i.visibility === "undefined" ? "" : i.visibility;
            if (
                i.status === "" &&
                !i.lost &&
                i.assigned === "" &&
        ...
        setTimeout(function () {
            $.post(admin_url + "leads/bulk_action", i)
                .done(function () {
                    ...
                    alert_float("success", "Bulk action performed successfully");
                })
                .fail(function (e) {
                    $(e).removeClass("disabled");
                    $("#leads_bulk_actions").modal("hide");
...
$('#leads_bulk_actions').on('show.bs.modal', function () {

    $("#re-assignation_div").hide();
    $("#bulk_change").show();

    $('#leads_bulk_actions').find("select").each(function () {
        $(this).val('');
        $(this).selectpicker('refresh');
    })
    if ($("#leads_bulk_actions").find('input[type="checkbox"]').is(":checked")) {
        $("#leads_bulk_actions").find('input[type="checkbox"]').prop('checked', false);
        // $("#bulk_change").toggleClass();
    }
$("#bulk_reference_name").val('');
    $('#leads_bulk_actions').find('.tagit li.ui-widget-content').remove();
    $('#leads_bulk_actions').find('.tagit li input').attr("placeholder", "Tags");
        ...
        if (n.length) {
            var s = {
                ajax: {
                    ...
                    data: function () {
                        let lead_type = $("#leads-filter-source").find("#lead_type").val();
                        if (lead_type == undefined) {
                            lead_type = "";
                        }
                        var e = {};
                        return e.ttt = 1, e.l = lead_type, e.type = t, e.rel_id = "", e.q = "{{{q}}}", void 0 !== a && jQuery.extend(e, a), e
                    }
                },
                locale: {
...
$("body").on("loaded.bs.select change", "select.ajax-search", function (e) {
    ...
    if (s && $("#search_input").val(s).trigger("keyup"), $("body").on("blur", "#timesheet_duration", function () {
        ...
        if ($("body .lead-view").toggleClass("hide"), t.toggleClass("hide"), !t.hasClass("hide")) {
            var a = $("#lead-modal").find("#address"),
                i = a.length > 0 && a[0].scrollHeight === desiredScrollHeight ? a[0].scrollHeight : "";
    ...
    var r = {
        ...
        view_form: "[name='view_form[]']",
         sub_status: "[name='view_sub_status[]']",
        ...
        ghostStatus: "[name='ghostStatus']",
        ghostCount: "[name='ghostCount']",
        ...
        time_condition: "[name='time_condition']",
        reporting_persons: "[name='reporting_persons']",
    ...
    if ((table_leads = $("table.table-leads")).length) {
        ...
        0 < c.length && (p.push(c.index()), _.push(c.index())), _table_api = initDataTable(table_leads, admin_url + "leads/table", _, p, r, [0, "desc"]), _table_api && 0 < c.length && _table_api.on("draw", function () {
        ...
        // }), $.each(r, function (e, t) {
        //     $("select" + t).on("change", function () {
        //         $("[name='view_course[]']").prop("disabled", "lost" == $(this).val() || "junk" == $(this).val()).selectpicker("refresh"), table_leads.DataTable().ajax.reload().columns.adjust().responsive.recalc()
        //     })
        // }), $.each(r, function (e, t) {
        //     $("select" + t).on("change", function () {
        //         $("[name='view_degree[]']").prop("disabled", "lost" == $(this).val() || "junk" == $(this).val()).selectpicker("refresh"), table_leads.DataTable().ajax.reload().columns.adjust().responsive.recalc()
        //     })
        // }),
        //     $.each(r, function (e, t) {
        //         $("select" + t).on("change", function () {
        //             $("[name='view_source[]']").prop("disabled", "lost" == $(this).val() || "junk" == $(this).val()).selectpicker("refresh"), table_leads.DataTable().ajax.reload().columns.adjust().responsive.recalc()
        //         })
        //     })
    }
    
        if ((table_leads_new = $("table.table-leads-new")).length) {
            
        var c = table_leads_new.find("#th-consent"),
            p = [0],
            _ = [0, table_leads_new.find("#th-assigned").index()];
        0 < c.length && (p.push(c.index()), _.push(c.index())), _table_api = initDataTable(table_leads_new, admin_url + "leads/table_neww?check=1", _, p, r, [0, "desc"]), _table_api && 0 < c.length && _table_api.on("draw", function () {
            var e = table_leads_new.find("tbody tr");
            $.each(e, function () {
                $(this).find("td:eq(3)").addClass("bg-light-gray")
            })
        })
...
function delete_leads(leadid) {
    if (confirm_delete()) {
        var data = { id: leadid };
        $.post(admin_url + "leads/delete_lead", data).done(function () {
            if (table_leads != undefined) {
                table_leads.DataTable().ajax.reload(null, !1);
            }
            alert_float("success", "Lead delete successfully");
        }).fail(function (e) {
            $(e).removeClass("disabled");
            alert_float("danger", e.responseText)
...
$('body').on('submit', '#lead-transfer', function (event) {
    event.preventDefault(); // Prevent the default form submission
    let lead_type = $("#transfer_lead_type").selectpicker('val');
    let lead_assign = $("#transfer_lead_assign").selectpicker('val');
    let reason = $("#reason").val(); // textarea
    if (!lead_type) {
        alert_float("danger", "Select Lead type");
        return false;
    }
    if (!lead_assign) {
        alert_float("danger", "Select Assigned staff member");
        return false;
    }
    if (reason == "") {
        alert_float("danger", "Reason is not empty.");
        return false;
    }
        ...
        if (response.success) {
            ...
            if (response.lead_id !== undefined && response.lead_id !== '') {
                ...
                if ($(".lead-transfer-table").length > 0 && $(".lead-transfer-table").is(':visible')) {
                    change_lead_request_table();
                }
                // init_lead(response.lead_id);
...
var lead_transfer_xhr = null;

function update_lead_transfer(leadid, type, id, assigned = "", status, source = "") {
    if (lead_transfer_xhr != null) {
        lead_transfer_xhr.abort();
    }
    lead_transfer_xhr = $.ajax({
        type: "POST",
        ...
        success: function (response) {
            if (response.success) {
                ...
                if (response.id !== undefined && response.id !== '') {
                       $("#lead-modal").modal('hide');
                    table_leads.DataTable().ajax.reload(null, !1)
                    if ($("#lead-modal").hasClass("in")) {
                        // init_lead_modal_data(response.id);
                    }
                    if ($(".lead-transfer-table").length > 0 && $(".lead-transfer-table").is(':visible')) {
                        change_lead_request_table();
                    }
...
var lead_transfer_xhr_update = null;
function change_transfer_lead_staff(lead_id, transfer_lead_id, transfer_lead_type = "", transfer_lead_assign = "") {

    if (lead_transfer_xhr_update != null) {
        lead_transfer_xhr_update.abort();
    }
    lead_transfer_xhr_update = $.ajax({
        type: "POST",
        url: "leads/add_lead_transfer_request",
        data: {
            lead_id: lead_id,
            transfer_lead_assign: transfer_lead_assign,
            transfer_lead_type: transfer_lead_type,
        ...
        success: function (response) {
            ...
            if (response.success) {
                alert_float("success", response.message);
                if ((response.id !== undefined && response.id !== '') || (response.lead_id !== undefined && response.lead_id !== '')) {
                    // // console.log(response);
                    // // console.log(response.id);
                    // // console.log(response.lead_id);
                    $("#lead-modal").modal('hide');
                    table_leads.DataTable().ajax.reload(null, !1)
                    if ($(".lead-transfer-table").length > 0 && $(".lead-transfer-table").is(':visible')) {
                        change_lead_request_table();
                    ...
                    if ($("#lead-modal").hasClass("in")) {
                        if (response.id != undefined) {
                            init_lead_modal_data(response.id, true, true);
                        }
                        else if (response.lead_id != undefined) {
                            init_lead_modal_data(response.lead_id, true, true);
                        }
                    }
...
function edit_lead_request(leadid) {
    ...
    // console.log(reason);
    // if (assign != '') {
    //     $("#transfer_lead_assign").val(assign).selectpicker('refresh');
    // }
    // if (lead_type != '') {
    //     $("#transfer_lead_type").val(lead_type).selectpicker('refresh');
    // }
    // if (reason != '') {
    //     $("#reason").val(reason);
    // }
...
//         $('input[name="mass_re-assignation"]').prop("checked", false);
...
function show_media_files(fileUrl = "", Type = "") {
    // // console.log("okkkkk");
    let mediaContent = "";
    let fileType = getFileType(fileUrl);
    if (fileType.startsWith("image")) {
        mediaContent = `<img src="${fileUrl}" class="img-fluid" style="width: -webkit-fill-available;" alt="Preview">`;
    } else if (fileType.startsWith("video")) {
        mediaContent = `<video controls class="w-100"><source src="${fileUrl}" type="video/mp4">Your browser does not support video.</video>`;
    } else if (fileType.startsWith("audio")) {
        mediaContent = `<audio controls class="w-100"><source src="${fileUrl}" type="audio/mp3">Your browser does not support audio.</audio>`;
    } else if (fileType === "pdf") {
        mediaContent = `<iframe src="${fileUrl}" width="100%" height="500px"></iframe>`;
    } else {
        mediaContent = `<p>Preview not available for this file type.</p>`;
    }

    $("#mediaPreviewContent").html(mediaContent);
    $("#mediaPreviewModal").modal("show"); // Show modal
}
...
function download_media_files(fileUrl, type = "") {
    if (!fileUrl) {
    ...
    link.download = filename; // Default download name

    if (fileUrl.includes("Dummy")) {
        // Optionally add timestamp
        let timestamp = new Date().getTime();
        let extension = filename.includes(".") ? filename.split('.').pop() : "";
        filename = "Air-ticket_" + timestamp + (extension ? ("." + extension) : "");
        link.download = filename;
    }

    if (fileUrl.includes("Dummy")) {
        // Replace "dummy" with "air_ticket"
        // filename = filename.replace("dummy", "air_ticket");
...
async function whatsapp_message_send(client_id, type, s_university_id = "", s_university_name = "") {
    ...
    upload_data.append(csrfData.token_name, csrfData.hash);

    try {
        let response = await $.ajax({
            url: admin_url + "/clients/whatsapp_message_send",
            method: "POST",
            data: upload_data,
            contentType: false,
            processData: false
...
var stickyHeader = document.querySelector('.sticky-header thead');
var stickyFilter = document.querySelector('.sticky-fixed');
var initialTop = null;
var initialTop_filter = null;


function updateStickyPosition() {
    // const stickyHeader = document.querySelector('.your-sticky-header-class'); // make sure this matches your actual class or selector
    if (!stickyHeader) return; // Prevent errors if element not found

    const scrollTop = window.scrollY || document.documentElement.scrollTop;

    // Ensure initialTop is declared globally if not already
    if (typeof initialTop === 'undefined') {
        window.initialTop = null;
    }
...
function setfilter_fixed() {

    if (!stickyFilter) return;

    // Declare global initialTop if not already set
    if (typeof window.initialTop_filter === 'undefined' || window.initialTop_filter === null) {
        const rect = stickyFilter.getBoundingClientRect();
        window.initialTop_filter = rect.top + window.scrollY; // Get absolute top position
    }

    const scrollTop = window.scrollY || document.documentElement.scrollTop;

    if (scrollTop >= window.initialTop_filter) {
        stickyFilter.classList.add('is_fixed');
    } else {
        stickyFilter.classList.remove('is_fixed');
    }
}
...
$(document).ready(function () {
    ...
    // console.log("Validation Enble");

    loadValidations();

    $(".yearpicker").datepicker({
        showAnim: "slideDown",
        dateFormat: 'yy',               // year-only format
        changeMonth: false,
        changeYear: true,
        showButtonPanel: true,
        yearRange: `1950:${new Date().getFullYear() + 5}`, // Dynamically add 5 future years
        beforeShow: function (input, inst) {
            $(input).attr("readonly", "readonly");  // prevent typing
    ...
    const MAX_MB = 7; // Set maximum file size in MB

    setTimeout(function () {
        // console.log("Yearpicker Readonly");
        $(".yearpicker").attr("readonly", "readonly !important");
        ...
        $(document).on('change', 'input[type="file"]', function () {
            ...
            if (file && file.size > MAX_MB * 1024 * 1024) {
                ...
                alert_float("danger", `File size exceeds ${MAX_MB} MB. Please select a smaller or compressed file.`);
                return false;
            }
        });
    }, 1000);
